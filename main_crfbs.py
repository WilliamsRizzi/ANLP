...
RESULT_FILE_csv = 'crfbs_results.csv'
...
def main(
    ...
    :return: ( embedding_layer, epochs, words_loaded, labeling_threshold, window_size, subsample, target_count,
               ...
               max_iterations, all_possible_transitions, scale, cv, verbose_bs, n_jobs, n_iter,
    ...
    evals = evaluate_solution(model=baseline, eval_set=eval_set, target_keys=target_concepts)
    print '--done'
...
os.system('echo "%s" >> %s' % (
        main(embedding_layer='512', epochs='15', words_loaded=50000, labeling_threshold=4.5, window_size=2,
             subsample=None, target_count=7,
             min_count=None, max_count=None, composed_words=None, disjoint=None,
             train_test_evaluate=[ .6, .2, .2 ], algorithm='l2sgd', c1=None, c2=1.5, max_iterations=200,
             all_possible_transitions=True, scale=[.8, 2.5], cv=3, verbose_bs=True, n_jobs=-1, n_iter=3,
             average='weighted'
        ), RESULT_FILE_csv)
)
